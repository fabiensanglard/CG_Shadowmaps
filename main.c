...
//Camera position
float p_camera[3] = {20,19,-1};

//Camera lookAt
float l_camera[3] = {2,0,-10};

//Light position
float p_light[3] = {3,19,-35};
...
void setupMatrices(float position_x,float position_y,float position_z,float lookAt_x,float lookAt_y,float lookAt_z)
{
	...
	glLoadIdentity();
	gluPerspective(FOV,RENDER_WIDTH/RENDER_HEIGHT,6,70);
...
void update(void)
{
    ...
    //3652
	p_light[0] = light_mvnt * cos(4300/1000.0);
	p_light[2] = light_mvnt * sin(4300/1000.0);
}
...
void drawObjects(void)
{
	...
	glBegin(GL_QUADS);
	glVertex3f(-TABLE_DIMENSION,2,-TABLE_DIMENSION);
	glVertex3f(-TABLE_DIMENSION,2, TABLE_DIMENSION);
	glVertex3f( TABLE_DIMENSION,2, TABLE_DIMENSION);
	glVertex3f( TABLE_DIMENSION,2,-TABLE_DIMENSION);
	glEnd();
